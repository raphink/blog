...
```bash
$ cat /etc/default/hostapd | grep -v "^\(#\|$\)"
RUN_DAEMON="yes"
DAEMON_CONF="/etc/hostapd/hostapd.conf"

$ sudo cat /etc/hostapd/hostapd.conf | grep -v "^\(#\|$\)"
interface=ath0
bridge=wifi0
driver=madwifi
logger_syslog=-1
...
wpa=1
wpa_passphrase=My Great Passphrase for the Access Point
...
```bash
$ sudo cat /etc/default/dnsmasq | grep -v "^\(#\|$\)"
ENABLED=1

$ sudo cat /etc/dnsmasq.conf | grep -v "^\(#\|$\)"
